...
avro_schema_t p_avro_schema_build_acct_data(u_int64_t wtc, u_int64_t wtc_2)
{
  ...
  if (wtc_2 & COUNT_ADD_INFO)
    avro_schema_record_field_append(schema, "add_info", avro_schema_long());
  ...
  struct timeval *basetime, struct pkt_stitching *stitch, avro_value_iface_t *iface)
{
  ...
  if (wtc_2 & COUNT_ADD_INFO) {
    pm_avro_check(avro_value_get_by_name(&value, "add_info", &field, NULL));
    pm_avro_check(avro_value_set_string(&field, pbase->add_info));
  }
